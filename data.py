...
class DataGeneratorClassifier(keras.utils.Sequence):
    ...
    def __init__(self, list_IDs, batch_size, image_size, nb_images, data_path=DATASET_PATH, n_channels=NUMBER_OF_CHANNELS, shuffle=SHUFFLE_DATA):
        'Initialisation'
...
# à modifier 
    def __data_generation(self, list_IDs_temp):
        ...
        X = np.zeros((self.batch_size, self.nb_images, *self.image_size, self.n_channels))
        y = np.empty((self.batch_size), dtype=int)

        for i, ID in enumerate(list_IDs_temp):
            
            cap = cv2.VideoCapture(os.path.join(self.data_path, ID))
            
            for j in range(self.nb_images):
                ret, frame = cap.read()
                if ret:
                    X[i, j, :] = smart_resize(np.asarray(frame), self.image_size)
